...
export default class ListView extends React.Component {
  ...
  render() {
    ...
    const loading = this.props.data.isFetching ? "Fetching" : "Vehicle Reports";

    return (<div className="container">
      <h3>{loading}</h3>

      <RaisedButton secondary={true} label="Add" onClick={e => browserHistory.push(`/admin/vehicle-reports/new`)}/>
      ...
      <Table rowHeight={50} rowsCount={rows.length} width={1000} maxHeight={500} headerHeight={50}>
        ...
        <Column
          ...
          cell={({rowIndex, ...props}) => (
            <Cell {...props}>
              <RaisedButton label="Show" onClick={e => browserHistory.push(`/admin/vehicle-reports/${rows[rowIndex].id}`)}/>
