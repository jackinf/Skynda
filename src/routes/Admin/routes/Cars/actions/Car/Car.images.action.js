...
export function onMainImageUpload(e) {
  return (dispatch) => {
    imageUtil.imageFileToBase64(e.target.files[0], (base64File) => {
      dispatch(change(FORMS.CAR_FORM, "mainImageContainer.base64File", base64File));
    });
  }
}
...
export function onMainImageRemove(e) {
  return (dispatch) => {
    dispatch(change(FORMS.CAR_FORM, "mainImageContainer.base64File", null));
  ...
  return (dispatch, getState) => {
    acceptedFiles.forEach(file => {
      imageUtil.imageFileToBase64(file, (base64File) => {
        dispatch(arrayPush(FORMS.CAR_FORM, `images`, {imageContainer: {base64File}}));
      });
    });
  }
}
...
export function onFaultFileUpload(e, index) {
  return (dispatch) => {
    imageUtil.imageFileToBase64(e.target.files[0], (base64File) => {
      dispatch(change(FORMS.CAR_FORM, `faults[${index}].imageContainer.base64File`, base64File));
    });
  }
}
